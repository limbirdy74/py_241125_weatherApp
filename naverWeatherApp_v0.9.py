...
# v0.9 원하는 시간마다 갱신되는 기능 추가 --> 문제가 있음. 타이머 떄문에 꼬임
...
import threading  #  시간마다 갱신되는 기능 추가를 위한 모듈
...
class WeatherApp(QMainWindow, form_class):
    def __init__(self):
        ...
        self.statusBar().showMessage("네이버 날씨 앱 v0.9")
        ...
        self.area_input_edit.returnPressed.connect(self.reflashTimer)  # enter 입력시 작동

        self.reflashTimer()  # 시작하자 말자 쓰레딩 메소드를 호출. 경고창이 뜸
    ...
    # v0.9 처음 실행시 크롤링 안되는 문제를 해결하기 위해 브로커 함수를 사용. 누더기 된 듯
    def refrlashTimer_call_0(self):
        self.reflashTimer(0)

    def refrlashTimer_call_1(self):
        self.reflashTimer(1)


    # v0.9  원하는 시간마다 다시 크롤링하는 타이머 메소드
    def reflashTimer(self, messageFlag):
        if messageFlag == 0:  # 경고창 출력
            self.weather_search_call()  # 날씨 조회 메인 함수 호출
            messageFlag = 0
            self.timer = threading.Timer(5, self.reflashTimer_call_0).start()
            print("크롤링갱신1!")
        else:  # 경고창 출력 없음
            self.weather_search(1)  # 날씨 조회 메인 함수 호출
            messageFlag = 1
            self.timer = threading.Timer(5, self.reflashTimer_call_1).start()
            print("크롤링갱신2!")


    ### 스레드 사용시 반드시 해줘야함. 안하면 메모리에 남아 있게됨
    def closeEvent(self, a0, QCloseEvent=None):  #  프로그램 종료시 발생한는 이벤트 메소드
